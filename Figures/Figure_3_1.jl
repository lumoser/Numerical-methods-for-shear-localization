...
function mainSolver(Ny = 301, reltol = 1e-6, solver = "R5P" )
    ...
    if evoPlt
        function evoPltAdaptive!(fig::Figure, ax1::Axis, ax2::Axis, ax3::Axis, ax4::Axis)
            ...
            lines!(ax4, savedValues.saveval[j][1]  , xc/1e3, color = :green, linewidth = 3, linestyle=:solid, label = "η")
            lines!(ax3, abs.(savedValues.saveval[j][2]), xc/1e3, color = :purple, linewidth = 3,linestyle=:solid, label = "ϵ");
            lines!(ax2, savedValues.saveval[j][3] .*secpyear*1e2, x/1e3, color = :black, linewidth = 3, linestyle=:solid, label = "Vₓ");
        ...
        fig = Figure(size = (1600, 900), fontsize =32)
        ...
        ax3 = Axis(fig[1, 3], xlabel="Shear strain rate [1/s]", xscale = log10)#, limits = ((-35,0, nothing,nothing)))  
        ax4 = Axis(fig[1, 4], xlabel="Viscosity [Pa s]", xscale = log10)
        ...
        if p2f
            save("Figures/Figure_3_1.png", fig)
        end
